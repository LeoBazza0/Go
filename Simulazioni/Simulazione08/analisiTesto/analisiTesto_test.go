...
- un tipo Vocabolario  in cui salvare le stringhe (sequenze di caratteri separate da white spaces) di un testo
 e, per ciascuna, il numero di volte che essa appare nel testo

- un metodo String per Vocabolario che stampa il Vocabolario in ordine lessicografico,
un lemma per riga seguito da spazio,':', spazio e dalla sua frequenza. Ad esempio:
Lorem : 3
ever : 1
...
- una funzione
  ...
  che restituisce la frequenza più bassa, la frequenza più alta e la media delle frequenze delle stringhe
   del Vocabolario
...
- una funzione
	...
	che restituisce la lista delle stringhe contenute nel Vocabolario che iniziano
	con il carattere (runa) char
...
- una funzione
  ...
  legge il testo contenuto nel file e crea un Vocabolario delle stringhe contenute nel testo, ciascuna con
  la frequenza con cui appare nel testo;
  stampa il Vocabolario creato, i risultati restituiti dalla funzione statistiche e la lista di stringhe
  restituita dalla funzione selezione (vedi esempio).
  Se non viene passato un argomento per il nome del file e/o un carattere, il programma deve stampare
  il messaggio "indicare nome file e iniziale".
  Se si verifica un errore nell'apertura del file, il programma deve stampare l'errore restituito per
   la mancata apertura del file.

...
- una struttura 'Cisterna' con 3 campi float64 per le tre dimensioni (in cm)
(NOTA SERVIRA' SPECIFICARE NOME E ORDINE DEI CAMPI?) e un ulteriore campo float64 che indica il livello
(in cm) a cui arriva l'acqua nella cisterna

- una funzione

    variazione(cisterna *Cisterna, lt int) error

  che, se lt è positivo e c'è abbastanza spazio nella cisterna, versa lt litri (***) nella cisterna
  e restituisce errore nil, altrimenti non versa niente nella cisterna e restituisce un errore
  (vedi esempio) segnalando la capienza in litri ancora disponibile; se invece lt è negativo,
   se ci sono abbastanza litri, prende lt litri dalla cisterna e restituisce errore nil,
    altrimenti non prende niente dalla cisterna e restituisce un errore (vedi esempio)
	segnalando la disponibilità (in litri).
...
- una funzione main() che legge da linea di comando tre numeri (float64) che rappresantano larghezza,
 lunghezza e altezza (in quest'ordine) di una cisterna a base rettangolare.
Se i dati sono tutti > 0, crea una cisterna vuota (livello = 0) con quelle dimensioni e la stampa (vedi es.);
altrimenti segnala l'errore "tutti gli argomenti devono essere >0" (se anche uno solo è minore o uguale a 0)
 o "tutti gli argomenti devono essere numerici" (se anche uno solo non è numerico) e termina.
Poi legge da standard input una sequenza di interi, che rappresentano litri, terminata da 9999:
quando il numero letto è positivo, viene messo nella cisterna un numero di litri corrispondenti,
 viceversa quando il numero è negativo, i litri vengono tolti, sempre che ciò sia possibile, altrimenti viene
 segnalato un errore (senza però terminare).
